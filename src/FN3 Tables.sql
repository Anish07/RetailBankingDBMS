...
CREATE TABLE ACCOUNT_STRUCT (product_id VARCHAR2(100) PRIMARY KEY CHECK (REGEXP_LIKE(product_id, '^PROD[0-9]+')), name VARCHAR2(100), category VARCHAR2(100), structure VARCHAR2(3) CHECK (REGEXP_LIKE(structure, '^xx[0-9]')))
--
...
CREATE TABLE EMPLOYEE_INFO (employee_id VARCHAR2(100) PRIMARY KEY CHECK (REGEXP_LIKE(employee_id, '^EMPL[0-9]+')), employee_level NUMBER CHECK (employee_level BETWEEN 1 AND 5), customer_id VARCHAR2(100), CONSTRAINT fk_customer_info FOREIGN KEY (customer_id) REFERENCES CUSTOMERS(customer_id), CONSTRAINT fk_level_info FOREIGN KEY (employee_level) REFERENCES EMPLOYEE_PAY(employee_level))
